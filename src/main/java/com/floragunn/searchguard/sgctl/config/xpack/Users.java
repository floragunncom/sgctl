...
public record Users(
    ...
    public record User(
        ...
        public static User parse(DocNode config, Parser.Context parserContext, String username) throws ConfigValidationException{
            ...
            if(!config.containsKey("username")) vErrors.add(new MissingAttribute("username", config));
            if(!config.containsKey("password")) vErrors.add(new MissingAttribute("password", config));
            if(!config.containsKey("roles")) vErrors.add(new MissingAttribute("roles", config));
            if(!config.containsKey("attributes")) vErrors.add(new MissingAttribute("attributes", config));


            User user = new User(
                    username,
                    vNode.get("hash").asString(),
                    vNode.get("opendistro_security_roles").asListOfStrings(),
                    vNode.get("attributes").asMap()
            );
