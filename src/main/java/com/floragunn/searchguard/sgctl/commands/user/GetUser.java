...
@Command(name = "get-user", description = "Gets a user")
public class GetUser extends ConnectingCommand implements Callable<Integer> {
    ...
    public Integer call() {
        try (SearchGuardRestClient client = getClient().debug(debug)) {
            ...
            if (verbose || debug) {
                System.out.println("Getting a user: " + userName);
            }
            BasicResponse basicResponse = client.getUser(userName);
            System.out.println(basicResponse.getMessage());
            System.out.println(basicResponse.getAsString("data"));
